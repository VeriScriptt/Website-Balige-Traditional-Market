                                ...
                                @foreach ($stores as $store)
                                    <div class="col-md-3 col-sm-6 col-lg-2 mb-4">
                                        <div class="card h-100 border-0 shadow-sm" style="border-radius: 0.5rem; overflow: hidden;">
                                            <a href="{{ route('store', $store->user_id) }}" class="card-img-tiles" data-abc="true">
                                                
                                                <div class="inner">
                                                    <div class="main-img position-relative">
                                                        <img src="{{ asset('images/gambar_toko/' . $store->gambar_toko) }}" alt="{{ $store->nama_toko }}" class="card-img-top" style="width: 100%; height: 200px; object-fit: cover;">
                                                    </div>
                                                </div>
                                            </a>
                                            <div class="card-body text-center d-flex flex-column justify-content-between" style="padding: 1rem;">
                                                <div class="d-flex flex-column align-items-center">
                                                    <h5 class="card-title font-weight-bold mb-2" style="font-size: 1.1rem; text-align: center;">{{ $store->nama_toko }}</h5>
                                                    <h5 class="card-title font-weight-bold mb-2" style="font-size: 1.1rem; text-align: center;">{{ $store->lantai_toko }}</h5>
                                                    <!-- Tambahkan informasi tambahan tentang toko di sini, misalnya alamat, deskripsi, dll. -->
                                                </div>
                                                <a href="{{ route('store', $store->user_id) }}" class="btn btn-outline-primary btn-sm mt-auto" data-abc="true" style="border-radius: 20px;">Kunjungi Toko</a>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            <div class="mb-4">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-secondary btn-sm dropdown-toggle"
                                        id="dropdownMenuReference" data-toggle="dropdown">Atur</button>
